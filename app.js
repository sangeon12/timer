...
window.onload = function(){
    ...
    const totalH = $(".total-h");
    const totalM = $(".total-m");
    const totalS = $(".total-s");

    let timeLabel = [h, m, s];
    let countLabel = [totalH, totalM, totalS];

    let check = true;//시작 중지 버튼을 바꾸는데 사용
    let check2 = false;//타이머 중지를 위해 사용
    ...
    $("#start").on("click", e => {
        ...
        if(check){
            $("#start").html('<button class="icon-btn" id="stop"><i class="fas fa-stop"></i></button>');
            check = false;
        }else{
            $("#start").html('<button class="icon-btn" id="start"><i class="fas fa-play"></i></button>');
            check2 = true;
            check = true;   
        }
    });
    ...
    $("#saveBtn").on("click", e => {
        $(".spin").each( (idx, item) => {
            ...
            top = Math.abs(top / 40); 
            set(timeLabel, idx ,top);
            set(countLabel, idx ,top);
        });
        $("#popup").fadeOut(500);
    });

    function set(arr, idx, top){
        arr[idx].html( ("00" + top ).zero()  );
    }
}
